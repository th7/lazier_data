...
RSpec.describe LazierData do
  ...
  context 'processing with sub parts' do
    before do
      ...
      lazier_data.enum do |item, sub_parts|
        item.sub_parts.each { |sub_part| sub_parts << sub_part }
      end

      lazier_data[:sub_parts].each_slice(2) do |sub_parts_slice|
  ...
  context 'processing with a split' do
    before do
      lazier_data.split do |item, a, b, c|
        case item.type
        when :a
          a << item
        when :b
      ...
      lazier_data[:a].each_slice(2) do |a_items|
        spec_data.upsert(:a_items, a_items)
      end

      lazier_data[:b].each_slice(2) do |b_items|
        spec_data.upsert(:b_items, b_items)
      end

      lazier_data[:c].each_slice(2) do |c_items|
  ...
  context 'splitting, treating sub parts as many-to-many, more data, in uneven batch sizes' do
    ...
    before do
      ...
      lazier_data.split do |item, a_items, b_items, c_items|
        case item.type
        when :a
          a_items << item
        when :b
      ...
      %i[a_items b_items c_items sub_parts].each.with_index(2) do |item_type, batch_size|
        lazier_data[item_type].each_slice(batch_size, &upsert_to(item_type))
      end

      %i[a_items b_items c_items].each do |item_type|
        lazier_data[item_type].enum do |item, item_sub_parts|
          item.sub_parts.each do |sub_part|
            item_sub_parts << spec_data.new(item_id: item.id, sub_part_id: sub_part.id)
          end
        end
      ...
      lazier_data[:a_items][:item_sub_parts].each_slice(2, &upsert_to(:a_item_sub_parts))
      lazier_data[:b_items][:item_sub_parts].each_slice(5, &upsert_to(:b_item_sub_parts))
      lazier_data[:c_items][:item_sub_parts].each_slice(9, &upsert_to(:c_item_sub_parts))
    end
