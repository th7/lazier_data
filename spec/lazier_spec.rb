...
RSpec.describe Lazier do
  ...
  let(:upsert) do
    Proc.new do |type, items|
      ...
      if type == :sub_parts
        items.map { |item| item.fetch(:external_id) }
      else
        ids[type] ||= 0
  ...
  describe '#go' do
    ...
    context 'a simple slicing processor' do
      let(:base_inputs) { [{ type: :a }] }
      let(:reps) { 10 }
      let(:expected_upserts) do
        {
          a: [{ type: :a }].cycle(reps).to_a,
        }
      end

      before do
        subject.each_slice(2) do |item_slice|
          upsert.call(:a, item_slice.map { |item| item.slice(:type) })
        end
      end

      it 'produces the expected "upserts"' do
    ...
    #         { type: :b },
    #         { type: :c },
    #       ].cycle(reps).to_a.product(1.upto(reps).to_a).map do |item, a_id|
    #         item.merge(a_id:)
    #       end.to_enum.with_index(1).map do |item, id|
    #         item.merge(id:)
    #       end
    #     }
    #   end
    ...
    #   before do
    #     subject.each_slice(2) do |item_slice|
    #       ids = upsert.call(:a, item_slice.map { |item| item.slice(:type) })
    #       ids.zip(item_slice) { |id, item| item[:id] = id }
    #     end

    #     subject.enum do |item, sub_parts|
    #       item[:sub_parts].each do |sub_part|
    #         sub_part[:a_id] = item[:id]
    #         sub_parts << sub_part
    #       end
    #     end

    #     subject[:sub_parts].each_slice(2) do |sub_part_slice|
    #       ids = upsert.call(:sub_parts, sub_part_slice)
    #       ids.zip(sub_part_slice) do |id, sub_part|
    #         sub_part[:id] = id
    #       end
    #     end
    #   end
    ...
    context 'a complex processing setup' do
